...
lust.befores = {}
lust.afters = {}
lust.filter = nil  -- Pattern to filter test cases
...
function lust.it(name, fn)
    -- Skip this test if filter is set and name doesn't match
    if lust.filter and not name:lower():find(lust.filter:lower(), 1, true) then
        local color = string.char(27) .. '[33m'  -- Yellow color for skip
        print(indent() .. color .. 'SKIP' .. normal .. ' ' .. name)
        lust.skips = lust.skips + 1
        return
    end
...
local function eq(t1, t2, eps)
    ...
    if type(t1) == 'number' then 
        -- Handle infinity and NaN cases properly
        if t1 == t2 then return true end  -- Handles inf == inf, -inf == -inf, and normal numbers
        if t1 ~= t1 or t2 ~= t2 then return false end  -- NaN comparison (NaN ~= NaN is true)
